...
export function ocrToActions(data: MergedOCR): AIAction[] {
  ...
  if (Array.isArray(data.items) && data.items.length > 0) {
    return data.items.map((item) => buildActionFromItem(item));
  }
...
function buildActionFromItem(item: OCRItem): AIAction {
...
示例：
- 单笔 "微信支付 麦当劳 -35.00" → {"items":[{"type":"expense","amount":35,"category":"餐饮","description":"麦当劳"}]}
- 多笔 "超市：纸巾15，饮料28" → {"items":[{"type":"expense","amount":15,"category":"日用","description":"纸巾"},{"type":"expense","amount":28,"category":"餐饮","description":"饮料"}]}
