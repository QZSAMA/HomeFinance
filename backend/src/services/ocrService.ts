...
请识别图片中的所有交易（可能有多笔），并以 JSON 格式返回，格式为：
...
示例：
- 单笔：微信支付麦当劳 -35.00 → {"items":[{"type":"expense","amount":35,"date":"2026-07-23","category":"餐饮","description":"麦当劳"}]}
- 多笔：超市小票含多件商品 → {"items":[{"type":"expense","amount":15,"category":"日用","description":"纸巾"},{"type":"expense","amount":28,"category":"餐饮","description":"饮料"}]}
      ...
      items: Array.isArray(parsed.items)
        ? parsed.items
            .map((it: any) => ({
              ...
              type: it.type === 'income' || it.type === 'expense' ? it.type : undefined,
            }))
...
export function mergeOcrResults(
  ...
  const merged: ParsedOCR = {
    ...
    // 多笔交易：优先视觉的 items，回退 Tesseract 的 items
    items: vision.items ?? tesseract.items,
  };
  ...
  const hasVision = visionFields.some((f) => vision[f] !== undefined) || !!vision.items;
  const hasTesseract = tesseractFields.some((f) => tesseract[f] !== undefined) || !!tesseract.items;
