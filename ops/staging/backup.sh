...
mkdir -p "$OUT"
restore_services() {
  docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" start minio minio-proxy backend frontend
}
...
docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" stop minio minio-proxy
