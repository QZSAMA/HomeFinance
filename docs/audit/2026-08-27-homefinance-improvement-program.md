...
2026-09-03 进度记录：P1-F-01/02 的决策已由 ADR-0005 Accepted；`1d2872b` 已将 Family 时区以 additive migration、IANA 校验和创建后不可变触发器落地，`3d5db10` 已提供 family-local half-open PeriodWindow 纯服务并通过上海、闰年和 DST 测试，`7af352b` 已提供 Decimal-safe currency summary/reconciliation 纯服务，`78ece16` 已将这些语义接入四个报表端点并验证本地日界、混币种和现金流对账。预算/目标/compare/估值和 as-of 规则仍是后续门禁，不能据此宣称财务语义整体闭环。
