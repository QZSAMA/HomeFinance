...
The follow-up configuration adds a digest-only three-image manifest (backend, frontend, deterministic mock-AI), an Environment-gated SSH deployment job, GitHub/VPS deployment serialization, staged candidate activation with health and Prisma migration checks, workflow-side critical browser journeys, and failure diagnostics. It advances the active manifest only after browser verification. A post-migration failure deliberately remains a forward-recovery incident rather than automatically starting an older application image. Staging disables public registration; the browser suite alone supplies an Environment-held registration key through an encrypted SSH tunnel to the VPS loopback port, so the initial IP-based setup does not expose the key. Backup temporarily stops application writers and MinIO before creating the PostgreSQL custom dump plus named MinIO-volume archive with checksums. Restore rehearsal waits for isolated dependencies, then checks API health, Prisma migration state and MinIO bucket readability without touching the active staging database.
