...
### 1.1 2026-09-01 实施状态同步

本文件原先以 `main` 基线描述风险。Phase 1 当前分支已对 DATA-001、DATA-002、DATA-003、DATA-004 的部分入口完成回归验证：Income/Expense、Import、Recurring、普通 Asset CRUD，以及支持的 AI Income/Expense/Asset/Liability proposal confirmation 已接入事务化 Ledger/Balance/幂等路径；AI text/OCR proposal-producing paths 在确认前不写入账本。AI confirmation 与普通 Asset CRUD 的真实 PostgreSQL 证据覆盖 proposal 抢占、结果持久化、同 key replay、不同 key 冲突、过期/篡改/角色负向、版本 CAS、并发矩阵和故障回滚。

以下基线段落仍保留为问题发现记录；凡涉及 AI 确认“尚未实现”的描述，均只表示基线，不能解释为当前分支状态。当前仍开放：legacy raw `/execute-actions` 兼容入口收口、普通 Liability HTTP CRUD、Balance 查询/估值与币种换算、外部 Redis/MinIO/Compose、production-like upgrade/restore、浏览器 E2E、staging/release observation，以及 semantic Graphify refresh。
...
|---|---|---|---|
| 收入创建/更新/删除 | route 调用 `ledgerApplicationService`，由 coordinator 统一持久化 | `backend/src/routes/incomes.ts:93-153`；`backend/src/services/ledgerApplicationService.ts` | 已有事务、幂等、审计和 finance revision 合同 |
| 支出创建/更新/删除 | route 调用 `ledgerApplicationService`，由 coordinator 统一持久化 | `backend/src/routes/expenses.ts:93-153`；`backend/src/services/ledgerApplicationService.ts` | 已有事务、幂等、审计和 finance revision 合同 |
| 普通资产 | GET 仍直接读 Prisma；POST/PUT/DELETE 已通过 Balance mutation/coordinator service | `backend/src/routes/assets.ts:36-220`；`backend/src/services/balanceMutationService.ts`；`backend/src/services/financialMutationCoordinator.ts` | 事务、幂等、审计、cacheVersion 与版本 CAS 已有 route contract 及真实 PostgreSQL 证据；估值/币种语义仍开放 |
| 普通负债/预算/目标 | 各 route 仍直接 create/update/delete（负债另有局部 membership helper） | `backend/src/routes/liabilities.ts:66-161`；`backend/src/routes/budgets.ts:123-207`；`backend/src/routes/goals.ts:106-185` | 不是同一个 ledger entry，但仍绕过统一 mutation policy/cache invalidation；普通 Liability CRUD 是下一迁移目标 |
| 定期执行 | route 调用 `recurringService.executeRecurring`，由 execution/coordinator 统一提交 | `backend/src/routes/recurring.ts:126-165`；`backend/src/services/recurringService.ts` | exactly-once、due/active/endDate、失败回滚已有本地 PostgreSQL 证据；规则定义 CRUD 仍为独立管理路径 |
| 导入确认 | route 调用 `importService.confirmImportBatch`，由 coordinator 统一提交 | `backend/src/routes/import.ts:87-96`；`backend/src/services/importService.ts` | batch 校验、原子提交、幂等和 replay 已有证据；上传资源边界仍待外部验证 |
| 文本/OCR AI mutation | proposal-producing path 持久化 server-owned proposal；确认由 proposal coordinator 调用 Ledger/Balance | `backend/src/routes/ai.ts`；`backend/src/services/aiProposalConfirmationService.ts` | 支持的 Income/Expense/Asset/Liability confirmation 不在确认前写入；legacy raw `/execute-actions` adapter 仍待收口 |
...
**当前实现状态**：提交 `5a564c9` 已将 text mutation 改为 proposal-only；提交 `732eafd`、`7f6c366`、`cdfcb67`、`d945e65`、`c769a78` 已分别提供 server-owned proposal/item 持久化、来源/哈希/版本/过期合同、支持的 Income/Expense 原子确认、结果/刷新恢复；提交 `fc50633`、`d204201`、`bdabe7b` 又将 `create_asset`/`create_liability` 通过 transaction-scoped Balance service 接入同一 proposal confirmation transaction；提交 `310e6ca`、`d180d60`、`c644d50` 已将普通 Asset CRUD 接入 Balance mutation/coordinator boundary，并增加版本 CAS、幂等、审计和 cacheVersion。当前 route/integration 测试和本地 PostgreSQL 证据表明，支持的确认路径与普通 Asset mutation 不会绕过 Ledger/Balance；legacy `/execute-actions` 仍作为严格 proposal confirmation adapter 保留，普通 Liability CRUD、Balance 查询/估值、币种换算和真实外部依赖仍未验证。
...
**基线根因**：文本 chat 将模型输出直接连接到 mutation executor；提议、确认、动作 schema 校验和最终账本提交没有分层。当前支持的 AI proposal path 已完成分层，但旧兼容入口仍是收口与删除前的残余风险。
...
**基线风险**：AI 误判、viewer 越权、proposal 篡改和批量部分成功均可改变财务事实，而调用方无法安全重试。当前已验证的支持路径已通过相应负向、原子性、replay 和 concurrency 检查；残余风险集中在 legacy adapter、Balance mutation、外部依赖和发布环境观察。
...
**当前退出门禁**：支持的 Income/Expense/Asset/Liability confirmation 以及普通 Asset CRUD 已有 proposal 篡改、过期、类型/字段校验、同 key replay、版本冲突、并发 claim、DB rollback 和主要角色/家庭边界的 focused 或本地 PostgreSQL 证据；完整 Phase 1 仍不能关闭：legacy raw `/execute-actions` 收口、普通 Liability CRUD、Balance 查询/估值与币种换算、provider timeout/外部依赖、浏览器 E2E、staging/release observation、global branch coverage 和 semantic Graphify refresh 仍开放。
...
HomeFinance 已有足够的领域模型和测试基础来渐进修复，不需要以微服务重写作为前提。基线的“路由直接写表”结构曾是财务一致性的系统性风险；当前普通收入/支出、Import、Recurring、普通 Asset CRUD 和支持的 AI Income/Expense/Asset/Liability confirmation 已围绕事务化 Ledger/Balance、幂等与审计事实逐步收口。legacy AI adapter、普通 Liability CRUD、Balance 查询/估值/币种换算、外部依赖与发布观察仍需完成，不能据此宣称所有财务写入口已经统一。

当前的后续顺序是：先迁移普通 Liability CRUD 到统一 policy/Balance service，再收口 legacy `/execute-actions`，并冻结 Balance 查询/估值/币种语义；随后完成 Redis/MinIO/Compose、生产样本升级恢复、浏览器 E2E、staging/release observation 和 Graphify semantic refresh。同时保持同一提交路径更新 family finance revision，报告缓存只做派生优化。失败、重试和并发必须返回可解释状态，而不是让调用方从通用 500 猜测是否安全重试。
