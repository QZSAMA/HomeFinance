...
| HF-AI-001 | P1 | 基线文本 AI 动作立即执行且 viewer 可触发；当前 LOCALLY_REMEDIATED / PASS-MOCK + PASS-REAL | text/OCR proposal-producing paths 已停止确认前自动执行；server-owned proposal persistence、proposalId/version/hash confirmation、事务内 Ledger/Audit/result/idempotency 以及 replay/concurrency/negative matrix 已有真实 PostgreSQL route 证据；刷新后 pending/EXECUTED 前端状态有组件测试。旧 direct executor 已移除，但严格兼容 adapter、Balance actions、外部基础设施、浏览器 E2E 与发布观察仍未闭环 | 基线 `ai.ts:132-175,457-468`；当前 `5a564c9`、`732eafd`、`7f6c366`、`cdfcb67`、`d945e65`、`e295e54`、`c769a78`、`6704300`、`9e89525`、AI proposal tests/integration、ADR-0004 |
...
2026-09-01 remediation status：HF-AI-001 已部分修复。`5a564c9` 通过 failing-then-passing route contract 证明 text chat 返回 `proposedActions` 且确认前 `executeActions` 零调用；`732eafd` 加入 server-owned AiProposal/AiProposalItem、原始/确认 payload/hash、version/status/expiry/source/result 和有序 action 合同；`7f6c366` 将 text、OCR、image-only 和 image-plus-text proposal-producing paths 接入 server-owned persistence；`cdfcb67`、`d945e65`、`c769a78` 完成支持的 Income/Expense 原子确认、幂等/并发/负向矩阵、历史刷新恢复和 EXECUTED 状态展示；`6704300` 移除旧的 direct Prisma executor；`9e89525` 建立 Playwright/Compose 验证基础设施，但本机 Docker preflight 阻塞实际执行。真实 PostgreSQL HTTP 证据覆盖 proposal/items、来源归属、viewer/non-member 早拒绝、确认零副作用、Ledger/Audit/result/idempotency 事务与重放。该风险尚未关闭：严格 `/execute-actions` 兼容 adapter、Balance actions、外部基础设施、浏览器 E2E 和发布观察仍未闭环。
