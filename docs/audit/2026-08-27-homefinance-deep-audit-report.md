...
| HF-AI-001 | P1 | 基线文本 AI 动作立即执行且 viewer 可触发；当前 PARTIALLY_REMEDIATED / PASS-MOCK + PASS-REAL schema/route | text/OCR proposal-producing paths 已停止确认前自动执行，viewer/non-member 由统一 middleware 和 service membership 拒绝，server-owned proposal persistence 已建立并有真实 PostgreSQL route 证据；legacy raw-actions confirm 仍缺 proposal ownership、原子性和真实并发证据 | 基线 `ai.ts:132-175,457-468`；当前 `5a564c9`、`732eafd`、`7f6c366`、AI proposal tests/integration、ADR-0004 |
