...
| HF-AI-001 | P1 | 基线文本 AI 动作立即执行且 viewer 可触发；当前 LOCALLY_REMEDIATED / PASS-MOCK + PASS-REAL | text/OCR proposal-producing paths 已停止确认前自动执行；server-owned proposal persistence、proposalId/version/hash confirmation、事务内 Income/Expense/Asset/Liability confirmation、Ledger/Balance/Audit/result/idempotency 以及 replay/concurrency/negative matrix 已有真实 PostgreSQL route 证据；刷新后 pending/EXECUTED 前端状态有组件测试。旧 direct executor 已移除，但严格兼容 adapter、普通 Liability CRUD、Balance 查询/估值/币种换算、外部基础设施、浏览器 E2E 与发布观察仍未闭环；普通 Asset CRUD 已在 P1-A-08 接入事务化 Balance mutation boundary | 基线 `ai.ts:132-175,457-468`；当前 `5a564c9`、`732eafd`、`7f6c366`、`cdfcb67`、`d945e65`、`e295e54`、`c769a78`、`6704300`、`9e89525`、`fc50633`、`d204201`、`bdabe7b`、`310e6ca`、`d180d60`、`c644d50`、AI/Asset proposal tests/integration、ADR-0004、ADR-0007 |
...
基线 coverage 曾使全局门槛失败：statements 53.78%、branches 39.72%、functions 55.21%、lines 54.07%。Phase 1 当前分支已通过新增行为测试改善为 50 suites / 417 tests，statements 75.81%、branches 60.04%、functions 70.85%、lines 76.67%，达到未降低的 60% 全局门槛；但测试输出仍有既有 OCR/MinIO console warning，不能据此宣称外部服务或浏览器 E2E 已验证。普通 Asset CRUD 已迁移至统一 Balance mutation/coordinator boundary；`liabilities.ts` 的普通 CRUD 迁移仍是后续工作，CI 是否把 coverage/外部集成/E2E 作为真实合并门禁仍需持续确认。
