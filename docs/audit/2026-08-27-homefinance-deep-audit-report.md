...
| HF-AI-001 | P1 | 基线文本 AI 动作立即执行且 viewer 可触发；当前 LOCALLY_REMEDIATED / PASS-MOCK + PASS-REAL | text/OCR proposal-producing paths 已停止确认前自动执行；server-owned proposal persistence、proposalId/version/hash confirmation、事务内 Income/Expense/Asset/Liability confirmation、Ledger/Balance/Audit/result/idempotency 以及 replay/concurrency/negative matrix 已有真实 PostgreSQL route 证据；刷新后 pending/EXECUTED 前端状态有组件测试。旧 direct executor 已移除；普通 Asset CRUD 已在 P1-A-08、普通 Liability CRUD 已在 P1-A-09 接入事务化 Balance mutation boundary。严格兼容 adapter、Balance 查询/估值/币种换算、外部基础设施、浏览器 E2E 与发布观察仍未闭环 | 基线 `ai.ts:132-175,457-468`；当前 `5a564c9`、`732eafd`、`7f6c366`、`cdfcb67`、`d945e65`、`e295e54`、`c769a78`、`6704300`、`9e89525`、`fc50633`、`d204201`、`bdabe7b`、`310e6ca`、`d180d60`、`c644d50`、`03eb48b`、`aee8b1e`、`aea0612`、`a9b33fb`、`2525cff`、`9770bf1`、`62cabf7`、AI/Asset/Liability proposal tests/integration、ADR-0004、ADR-0007 |
...
2026-09-01 remediation status：HF-AI-001 已部分修复。`5a564c9` 通过 failing-then-passing route contract 证明 text chat 返回 `proposedActions` 且确认前 `executeActions` 零调用；`732eafd` 加入 server-owned AiProposal/AiProposalItem、原始/确认 payload/hash、version/status/expiry/source/result 和有序 action 合同；`7f6c366` 将 text、OCR、image-only 和 image-plus-text proposal-producing paths 接入 server-owned persistence；`cdfcb67`、`d945e65`、`c769a78` 完成支持的 Income/Expense 原子确认、幂等/并发/负向矩阵、历史刷新恢复和 EXECUTED 状态展示；`6704300` 移除旧的 direct Prisma executor；`9e89525` 建立 Playwright/Compose 验证基础设施，但本机 Docker preflight 阻塞实际执行。`fc50633`、`d204201`、`bdabe7b` 进一步将 `create_asset`/`create_liability` 通过 transaction-scoped Balance service 接入同一 proposal confirmation transaction；`310e6ca`、`d180d60`、`c644d50` 将普通 Asset CRUD 接入同一 Balance mutation/coordinator boundary，并增加版本 CAS、幂等、审计和 cacheVersion 合同；`03eb48b`、`aee8b1e`、`aea0612`、`a9b33fb`、`2525cff`、`9770bf1`、`62cabf7` 完成普通 Liability CRUD 的同类迁移、Liability.version additive migration、route/adapter 合同和本地 PostgreSQL 验证。本地 PostgreSQL AI confirmation/route 25/25、Asset CRUD 4/4、Liability CRUD 5/5，完整 integration 13 suites / 85 tests 证明 Balance/Asset/Liability facts、proposal item result、AuditEvent、IdempotencyRecord 的原子提交、same-key replay、并发 claim、stale-version arbitration 和故障回滚。真实 PostgreSQL HTTP 证据覆盖 proposal/items、来源归属、viewer/non-member 早拒绝、确认零副作用、Ledger/Balance/Audit/result/idempotency 事务与重放。该风险尚未关闭：严格 `/execute-actions` 兼容 adapter、Balance 查询/估值/币种换算、外部基础设施、浏览器 E2E 和发布观察仍未闭环。
...
基线 coverage 曾使全局门槛失败：statements 53.78%、branches 39.72%、functions 55.21%、lines 54.07%。Phase 1 当前分支已通过新增行为测试改善为 50 suites / 430 tests，statements 76.14%、branches 60.01%、functions 71.42%、lines 77.09%，达到未降低的 60% 全局门槛；但测试输出仍有既有 OCR/MinIO console warning，不能据此宣称外部服务或浏览器 E2E 已验证。普通 Asset 与 Liability CRUD 已迁移至统一 Balance mutation/coordinator boundary；Balance 查询/估值/币种、CI 外部集成/E2E 合并门禁和发布观察仍需持续确认。
...
2026-09-08 P1-H-03 follow-up：在 `codex/p1-h03-release-rehearsal@ad38b67de87e7d360d1884b944ae9272759630bf` 的 Draft PR [#6](https://github.com/QZSAMA/HomeFinance/pull/6) 上，GitHub Actions run `34195623469` 完成了 PostgreSQL 16.15 的 disposable populated migration/backup/restore/forward-recovery rehearsal；同 SHA 的 CI `34195623427`、Browser E2E `34195623424` 与 Infrastructure Recovery `34195623440` 也成功。Ubuntu 24.04.4 / Node 20.20.2 runner 从 legacy migration cut `20260723030000_add_file_id_to_ai_conversation` 升级至 head `20260903100200_add_goal_contributions`；`pre-upgrade.dump` 为 33,011 bytes（SHA-256 `ad3ea82ee2aedf2bbaa72fe5f3b960fe27401020c4037f257c4d7efcde2cafce`），`current.dump` 为 78,904 bytes（SHA-256 `746422462089a8fa3ffa06733865935afd8cccef018692f19f75cb6180d3f118`）。九个 checkpoint 全部 PASS：legacy facts/manifest 保留，注入的 failure sentinel 与 corruption 在 restore 后消失并恢复精确 manifest，forward recovery 成功，重复 migrate 报告无 pending migrations，runner/workflow cleanup 为 true。API smoke 覆盖 401 未认证、403 非成员和 viewer 写入（零副作用）、家庭隔离、Income 幂等 replay、Import confirm、Recurring execution、AI proposal 显式确认、GoalContribution replay，以及 CNY Income 120 − Expense 113 = Net 7 对账。该证据仅证明 disposable rehearsal，不是 staging/production release 或 observation；受保护 `staging` Environment/reviewer、不可变 image digests、approved populated fixture、PostgreSQL/MinIO backup policy、observability、owner approval、30 个每分钟 observation samples、生产 RPO/RTO、历史估值/FX、durable cross-system atomicity 和 semantic Graphify refresh 仍开放。因此 P1-H-03 仍 `BLOCKED`/`AT_RISK`，P1-H-04 仍未启动，报告的“不适合直接公开生产”结论不变。
