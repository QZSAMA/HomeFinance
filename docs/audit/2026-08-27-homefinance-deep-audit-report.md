...
2026-09-08 P1-H-03 follow-up：在 branch SHA `c03d35ec9bcfd3adbb50fbd57c45043abf33d0f0` 的 Draft PR [#6](https://github.com/QZSAMA/HomeFinance/pull/6) 上，GitHub Actions run `34202444770` 完成了 PostgreSQL 16.15 的 disposable populated migration/backup/restore/forward-recovery rehearsal；同 SHA 的 CI `34202444766`、Browser E2E `34202444921` 与 Infrastructure Recovery `34202444902` 也成功。Ubuntu 24.04.4 / Node 20.20.2 runner 从 legacy migration cut `20260723030000_add_file_id_to_ai_conversation` 升级至 head `20260903100200_add_goal_contributions`；`pre-upgrade.dump` 为 33,011 bytes（SHA-256 `467cad823ff3b61cb4c9d1dd485e0e37c3d7d7dfab4a06355d623c2fa094feab`），`current.dump` 为 78,930 bytes（SHA-256 `2e971287da4459e39852e62368196639c07807aa7e861f992eb4b5c9c887d1f7`）。九个 checkpoint 全部 PASS：legacy facts/manifest 保留，注入的 failure sentinel 与 corruption 在 restore 后消失并恢复精确 manifest，forward recovery 成功，重复 migrate 报告无 pending migrations，runner/workflow cleanup 为 true。API smoke 覆盖 401 未认证、403 非成员和 viewer 写入（零副作用）、家庭隔离、Income 幂等 replay、Import confirm、Recurring execution、AI proposal 显式确认、GoalContribution replay，以及 CNY Income 120 − Expense 113 = Net 7 对账。该证据仅证明 disposable rehearsal，不是 staging/production release 或 observation；受保护 `staging` Environment/reviewer、不可变 image digests、approved populated fixture、PostgreSQL/MinIO backup policy、observability、owner approval、30 个每分钟 observation samples、生产 RPO/RTO、历史估值/FX、durable cross-system atomicity 和 semantic Graphify refresh 仍开放。因此 P1-H-03 仍 `BLOCKED`/`AT_RISK`，P1-H-04 仍未启动，报告的“不适合直接公开生产”结论不变。
