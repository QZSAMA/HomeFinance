...
| HF-AI-001 | P1 | 基线文本 AI 动作立即执行且 viewer 可触发；当前 LOCALLY_REMEDIATED / PASS-MOCK + PASS-REAL | text/OCR proposal-producing paths 已停止确认前自动执行；server-owned proposal persistence、proposalId/version/hash confirmation、事务内 Ledger/Audit/result/idempotency 以及 replay/concurrency/negative matrix 已有真实 PostgreSQL route 证据；刷新后 pending/EXECUTED 前端状态有组件测试。legacy raw-actions adapter、Balance actions、外部基础设施、浏览器 E2E 与发布观察仍未闭环 | 基线 `ai.ts:132-175,457-468`；当前 `5a564c9`、`732eafd`、`7f6c366`、`cdfcb67`、`d945e65`、`e295e54`、`c769a78`、AI proposal tests/integration、ADR-0004 |
...
2026-09-01 remediation status：HF-AI-001 已部分修复。`5a564c9` 通过 failing-then-passing route contract 证明 text chat 返回 `proposedActions` 且确认前 `executeActions` 零调用；`732eafd` 加入 server-owned AiProposal/AiProposalItem、原始/确认 payload/hash、version/status/expiry/source/result 和有序 action 合同；`7f6c366` 将 text、OCR、image-only 和 image-plus-text proposal-producing paths 接入 server-owned persistence，并通过真实 PostgreSQL HTTP 证据证明 proposal/items 写入、来源归属、viewer/non-member 早拒绝和四类财务表零变化。该风险尚未关闭：`/execute-actions` 仍接受客户端 raw actions，尚无 proposalId/version/hash 单事务 Ledger/Balance 确认、幂等并发、过期/篡改零副作用或 E2E 证据。
