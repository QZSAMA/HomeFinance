...
- 快照补充（2026-09-02，`cb75891`）：P1-A-09 进入 `GREEN_MINIMAL`，仅 Liability POST 已接入 Balance/coordinator，GET 使用集中式 family access；PUT/DELETE、Liability version migration/CAS 和真实 PostgreSQL 证据仍未完成。当前默认后端测试为 50 suites / 419 tests，`npm run build` 通过；本次提交未改变 coverage 结论，外部基础设施、Playwright、restore、release observation 和 semantic Graphify 仍按下文保持阻塞或未运行。
...
| P1-A-09 | TASK | P1-A | 迁移普通 Liability CRUD 到 Ledger/Balance | P0 | GREEN_MINIMAL | AT_RISK | API Agent | Technical Approver | hard:P1-A-03@REGRESSION_VERIFIED; exception:RepositoryOwner@2026-09-02 | route 无直接 Prisma mutation；member create/update/delete 具备事务、幂等、审计、cacheVersion 和版本 CAS；viewer/非成员/stale version 零副作用 | evidence/P1-A-09.md | ADR-0001, ADR-0002, ADR-0007 | `cb75891` 完成 POST 首片段和集中式 GET family access；PUT/DELETE 仍是直接 Prisma 路径，下一步补 version migration、update/delete service/route RED→GREEN 和 PostgreSQL integration；保持单入口、不双写、不扩大到估值/汇率 | 2026-09-02 |
...
| P1-G-06 | GATE | P1-G | coverage/lint/advisory 门禁 | P1 | IN_REVIEW | AT_RISK | QA/Evidence Owner | Repository Owner | hard:P1-0-07@REGRESSION_VERIFIED | coverage 真执行、warning 不增、high 有处置 | evidence/P1-G-06.md | — | 上次 coverage 快照为 50 suites / 417 tests、branches 60.04% PASS-REAL；本次新增 Liability 最小切片后行为测试为 50 suites / 419 tests，但尚未重跑 coverage，故门禁仍按 AT_RISK 处理；frontend lint/build 通过但保留 16 条既有 warning，backend/frontend high advisory 尚无本轮处置；Graphify semantic refresh 仍待可用 runner | 2026-09-02 |
