...
- 当前快照：2026-09-01；设计已批准，P1-B-03/04/05 已在 PostgreSQL 18.1 localhost:5433 完成真实的 key 冲突零写入、并发/重放和 revision 一致性回归：不同 payload 重用同 key 返回 409 且不新增事实/不推进 revision，20 路相同命令只使 `Family.cacheVersion` 前进一次，已完成的回放不新增事实也不前进 revision（PASS-REAL）；P1-B-04 在 `DomainError.cause` 保留未分类 Prisma 原始错误；P1-G-06 的安全关键 coordinator/ledger tests 将默认 backend coverage 提升到 34 suites / 295 tests、statements 63.71%、branches 47.10%、functions 64.06%、lines 64.00%，并锁定 malformed payload、成员记录错配、损坏回放和命令校验的零事务/零查询边界；早前的 Prisma adapter `Map`/`Set`/`RegExp` 契约和 MinIO controlled-degradation/shutdown 回归仍有效；P1-G-03 已为 ImportPage 建立 3 条 PASS-MOCK 交互合同：可访问文件选择、失败/成功状态和悬挂确认的本地防重复；P1-A-03 已以真实 PostgreSQL failure injection 证明空 resourceId 会回滚 Income、IdempotencyRecord 和 AuditEvent（PASS-REAL）；本轮 P1-A-01/04/05 在用户批准的依赖例外下将 Income/Expense 的 POST/PUT/DELETE 接入 Ledger、审计、幂等和版本条件写。`ledgerRoutes.phase1.integration.test.ts` 已以真实 HTTP→PostgreSQL 验证 Income 的 create/replay/update/stale/delete 和 Expense 的 create/update/stale/delete；`roleMethodMatrix.integration.test.ts` 已验证所有登记的 family-scoped mutation 入口对未认证/非成员/viewer/未知角色的 401/403 及零副作用，并补充 member/admin 对非账本稳定本地路径的正向 2xx，完整集成套件 7 suites / 41 tests 通过（PASS-REAL）。P1-C-01 已为 Import CSV 固化 10 MiB/10,000 行/512 字段长度限制及统一 413 `IMPORT_LIMIT_EXCEEDED` 合同；P1-C-02 已完成 ImportBatch/ImportRow additive schema、migration、约束和真实 PostgreSQL cascade/唯一性验证；P1-C-03 已将 CSV preview 接入服务端持有的 batch/row 持久化，保留旧数组响应并新增 CORS 可读的 `X-Import-Batch-Id`/`X-Import-Preview-Hash`，且真实 HTTP→PostgreSQL 已验证 family/actor/parser/status/row canonical payload；P1-C-04/C-05 已切换并验证 batch-only 原子 confirm、服务器 canonical rows、幂等 replay 和并发 claim。全量 coverage 执行 40 suites / 333 tests，所有行为断言通过，但 branch coverage 48.83% 未达 60%，前端 batch/hash 状态迁移、AI/upload 外部路径、对象隔离、upgrade/restore、staging/release 仍未完成。
...
| backend coverage | FAILED | 40 suites / 329 tests 行为断言通过；coverage 为 statements 69.68%、branches 51.69%、functions 69.68%、lines 70.24%，全局 branch threshold 60% 未达标；OCR/MinIO fixtures 有既有 console.warn |
| PostgreSQL migration/业务并发 | PASS-REAL | localhost:5433 `homefinance_phase1_test`；增量 migration、schema/约束/cascade、P1-B-04 20 路 coordinator replay、P1-B-05 revision/replay、P1-A authenticated-member HTTP CRUD 路由及 P1-G-02 全入口拒绝/非外部路径正向矩阵均已通过（6 suites / 36 tests） |
| Redis 故障恢复 | NOT_RUN | 现有主要为 mock |
| MinIO 生命周期 | NOT_RUN | 现有主要为 mock |
| Compose 全栈 | NOT_RUN | 当前未完成全栈验证 |
| Playwright | DESIGNED | 当前尚无脚本 |
| Phase 1 功能 | PASS-MOCK + PASS-REAL | app/server/db、Ledger/coordinator、schema/migration、ImportBatch/Row persistence 与 P1-B-04 adapter 已验证；Income/Expense POST/PUT/DELETE 路由已接入 Ledger，authenticated-member CRUD HTTP→PostgreSQL、所有登记 family mutation 入口的真实拒绝矩阵，以及 Asset/Liability/Budget/Goal/Recurring/Import/File delete 正向 member/admin 路径为 PASS-REAL；ImportBatch/Row route adoption、AI/upload 外部路径和 import/recurring/AI transactional adoption 仍未接入 |
...
| P1-G-06 | GATE | P1-G | coverage/lint/advisory 门禁 | P1 | RED_REPRODUCED | AT_RISK | QA/Evidence Owner | Repository Owner | hard:P1-0-07@REGRESSION_VERIFIED | coverage 真执行、warning 不增、high 有处置 | evidence/P1-G-06.md | — | 账本协调器新增 malformed payload、成员错配、损坏回放与非幂等冲突分支；Ledger command 在未查询 membership 前拒绝无效 amount/category/currency/source。34 suites/295 tests 通过；branch 47.10% 仍低于 60%。安装的 Graphify 仅 AST 增量且会覆盖审阅过的语义图，已恢复生成物；语义增量运行器待补。继续安全关键分支测试且不降低阈值 | 2026-08-31 |
...
|---|---|---|---|
| P1-D-04 | external:POSTGRES_TEST_ENV@AVAILABLE | QA/Evidence Owner / Release Owner | 本地 PostgreSQL 已可用；Recurring schema/service 完成后复查 |
