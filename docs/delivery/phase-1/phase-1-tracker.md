...
- 当前快照：2026-09-01；P1-A 已将 Income/Expense HTTP mutation 接入统一 Ledger，P1-B 已有数据库仲裁幂等与 20 路并发证据，P1-C 已完成服务端 Import batch/hash、原子 confirm 和前端迁移。P1-D 在提交 `370b2d9` 完成 Recurring exactly-once：`RecurringExecution` 以 `(recurringTransactionId, scheduledFor)` 唯一，同一 PostgreSQL transaction 内提交 execution、Ledger entry、规则推进、幂等和审计；20 个不同 key 对同一发生项只产生一条账目，inactive/future/endDate、强制回滚、正确 execution audit 和墓碑历史保留均为 PASS-REAL。后端 build、Prisma validate/format 通过；integration 8 suites / 47 tests 通过；coverage 执行 42 suites / 344 tests 且行为断言全过，但 branch 47.68% 未达 60%；前端 6 files / 15 tests、lint、build 通过，保留 16 条既有 warning 和 857.88 kB 主 chunk 警告。AI proposal-only、对象/外部基础设施、populated upgrade/restore、E2E、staging/release 仍未完成。
...
| backend coverage | FAILED | 42 suites / 344 tests 行为断言通过；coverage 为 statements 67.50%、branches 47.68%、functions 64.16%、lines 68.04%，全局 branch threshold 60% 未达标；OCR/MinIO fixtures 有既有 console.warn |
| PostgreSQL migration/业务并发 | PASS-REAL | PostgreSQL 18.1；增量/fresh migration、schema/约束/cascade、P1-B coordinator replay/revision、P1-A HTTP CRUD、P1-C atomic import、P1-D recurring 20 路不同 key occurrence 仲裁/回滚/墓碑，以及 P1-G-02 角色矩阵均已通过（8 suites / 47 tests） |
| Redis 故障恢复 | NOT_RUN | 现有主要为 mock |
| MinIO 生命周期 | NOT_RUN | 现有主要为 mock |
| Compose 全栈 | NOT_RUN | 当前未完成全栈验证 |
| Playwright | DESIGNED | 当前尚无脚本 |
| Phase 1 功能 | PASS-MOCK + PASS-REAL | app/server/db、Ledger/coordinator、schema/migration、Income/Expense route、Import batch confirm/client 和 Recurring exactly-once route/client 已验证；registered family mutation 拒绝矩阵及本地正向路径为 PASS-REAL。AI proposal-only/upload 外部路径、对象隔离、Compose/E2E 和 release observation 仍未闭环 |
...
| P1-G-03 | TASK | P1-G | 补齐前端 mutation 行为测试 | P1 | GREEN_MINIMAL | AT_RISK | Frontend Agent | QA/Evidence Owner | hard:P1-0-03@IN_REVIEW | error/loading/confirm/replay 可测 | evidence/P1-G-03.md | — | Import/AI 交互合同继续有效；Recurring service/page 新增 scheduledFor + stable-key failed-retry 合同。全前端 6 files / 15 tests、lint/build PASS；继续覆盖 reload/multi-tab/E2E 和 AI proposal-only | 2026-09-01 |
...
| P1-G-06 | GATE | P1-G | coverage/lint/advisory 门禁 | P1 | RED_REPRODUCED | AT_RISK | QA/Evidence Owner | Repository Owner | hard:P1-0-07@REGRESSION_VERIFIED | coverage 真执行、warning 不增、high 有处置 | evidence/P1-G-06.md | — | 后端 42 suites / 344 tests 行为断言通过；statements 67.50%、branches 47.68%、functions 64.16%、lines 68.04%，仍仅 branch 未达 60%。前端 lint PASS，16 条既有 warning；不降低阈值。Graphify semantic refresh 仍待可用 runner | 2026-09-01 |
| P1-H-01 | TASK | P1-H | 同步 API、ADR、memory、风险 | P0 | BACKLOG | ON_TRACK | Delivery DRI | Repository Owner | hard:P1-A-07@REGRESSION_VERIFIED | 长期事实和证据一致 | evidence/P1-H-01.md | ADR-0001~0006 | 文档随代码同提交；按变更更新 | 2026-09-01 |
