...
| backend coverage | FAILED | 37 suites / 322 tests 行为断言通过；coverage 为 statements 68.54%、branches 50.48%、functions 66.76%、lines 69.08%，全局 branch threshold 60% 未达标；OCR/MinIO fixtures 有既有 console.warn |
| PostgreSQL migration/业务并发 | PASS-REAL | localhost:5433 `homefinance_phase1_test`；fresh migration、schema/rollback、P1-B-04 20 路 coordinator replay、P1-B-05 revision/replay、P1-A authenticated-member HTTP CRUD 路由及 P1-G-02 全入口拒绝矩阵均已通过（5 suites / 32 tests） |
| Redis 故障恢复 | NOT_RUN | 现有主要为 mock |
| MinIO 生命周期 | NOT_RUN | 现有主要为 mock |
| Compose 全栈 | NOT_RUN | 当前未完成全栈验证 |
| Playwright | DESIGNED | 当前尚无脚本 |
| Phase 1 功能 | PASS-MOCK + PASS-REAL | app/server/db、Ledger/coordinator、schema/migration 与 P1-B-04 adapter 已验证；Income/Expense POST/PUT/DELETE 路由已接入 Ledger，authenticated-member CRUD HTTP→PostgreSQL 和所有登记 family mutation 入口的真实拒绝矩阵为 PASS-REAL；非账本资源的正向全角色矩阵、import/recurring/AI adoption 仍未接入 |
...
| P1-G-02 | GATE | P1-G | 完成角色×方法×入口矩阵 | P0 | REFACTORED | AT_RISK | Security Reviewer | Repository Owner | hard:P1-0-04@READY | 401/403/200/2xx 且零副作用 | evidence/P1-G-02.md | ADR-0001 | 全入口拒绝矩阵 PASS-REAL；补非账本资源 member/admin 正向 2xx、对象隔离和外部副作用核验后再验证 | 2026-09-01 |
