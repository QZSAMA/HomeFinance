...
- 当前快照：2026-08-28；设计已批准，书面规格待审阅；生产代码尚未修改
...
| P1-G-00 | TASK | P1-G | 分离 app/server/db 启动边界 | P0 | READY | ON_TRACK | Delivery DRI | Technical Approver | hard:P1-0-03@DONE | app import 无 listener/Redis/MinIO 副作用 | evidence/P1-G-00.md | ADR-0001 | RED: `backend/src/app.test.ts` / `imports without opening a listener or initializing external services` / `npm test -- --runInBand src/app.test.ts`; RED 后拆分 app/server/db | 2026-08-28 |
...
| P1-G-01 | GATE | P1-G | 真实 PostgreSQL migration/事务 | P0 | BLOCKED | AT_RISK | QA/Evidence Owner | Release Owner | external:POSTGRES_TEST_ENV@AVAILABLE | migration、trigger、rollback、concurrency PASS-REAL | evidence/P1-G-01.md | ADR-0001, ADR-0002 | 环境不可用保持 BLOCKED | 获得测试数据库 | 2026-08-28 |
...
| P1-G-06 | GATE | P1-G | coverage/lint/advisory 门禁 | P1 | BACKLOG | AT_RISK | QA/Evidence Owner | Repository Owner | hard:P1-0-07@REGRESSION_VERIFIED | coverage 真执行、warning 不增、high 有处置 | evidence/P1-G-06.md | — | 分组回退 lockfile；做 audit 基线 | 2026-08-28 |
...
|---|---|---|---|
| P1-B-04、P1-C-05、P1-D-04、P1-G-01 | external:POSTGRES_TEST_ENV@AVAILABLE | QA/Evidence Owner / Release Owner | 环境可用后立即复查 |
