...
| backend coverage | FAILED | 39 suites / 328 tests 行为断言通过；coverage 为 statements 69.68%、branches 51.69%、functions 69.68%、lines 70.24%，全局 branch threshold 60% 未达标；OCR/MinIO fixtures 有既有 console.warn |
| PostgreSQL migration/业务并发 | PASS-REAL | localhost:5433 `homefinance_phase1_test`；fresh migration、schema/rollback、P1-B-04 20 路 coordinator replay、P1-B-05 revision/replay、P1-A authenticated-member HTTP CRUD 路由及 P1-G-02 全入口拒绝/非外部路径正向矩阵均已通过（5 suites / 33 tests） |
| Redis 故障恢复 | NOT_RUN | 现有主要为 mock |
| MinIO 生命周期 | NOT_RUN | 现有主要为 mock |
| Compose 全栈 | NOT_RUN | 当前未完成全栈验证 |
| Playwright | DESIGNED | 当前尚无脚本 |
| Phase 1 功能 | PASS-MOCK + PASS-REAL | app/server/db、Ledger/coordinator、schema/migration 与 P1-B-04 adapter 已验证；Income/Expense POST/PUT/DELETE 路由已接入 Ledger，authenticated-member CRUD HTTP→PostgreSQL、所有登记 family mutation 入口的真实拒绝矩阵，以及 Asset/Liability/Budget/Goal/Recurring/Import/File delete 正向 member/admin 路径为 PASS-REAL；AI/upload 外部路径和 import/recurring/AI transactional adoption 仍未接入 |
