...
| backend coverage | FAILED | 37 suites / 318 tests 行为断言通过；coverage 为 statements 68.13%、branches 49.91%、functions 66.37%、lines 68.64%，全局 branch threshold 60% 未达标；OCR/MinIO fixtures 有既有 console.warn |
| PostgreSQL migration/业务并发 | PASS-REAL | localhost:5433 `homefinance_phase1_test`；fresh migration、schema/rollback、P1-B-04 20 路 coordinator replay、P1-B-05 revision/replay 一致性及 P1-A authenticated-member HTTP CRUD 路由均已通过（4 suites / 29 tests） |
| Redis 故障恢复 | NOT_RUN | 现有主要为 mock |
| MinIO 生命周期 | NOT_RUN | 现有主要为 mock |
| Compose 全栈 | NOT_RUN | 当前未完成全栈验证 |
| Playwright | DESIGNED | 当前尚无脚本 |
| Phase 1 功能 | PASS-MOCK + PASS-REAL | app/server/db、Ledger/coordinator、schema/migration 与 P1-B-04 adapter 已验证；Income/Expense POST/PUT/DELETE 路由已接入 Ledger，且 authenticated-member CRUD HTTP→PostgreSQL 为 PASS-REAL；完整真实角色矩阵仍为 PASS-MOCK/NOT_RUN，import/recurring/AI adoption 仍未接入 |
