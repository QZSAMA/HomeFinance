...
- 当前快照：2026-09-01；P1-A 已将 Income/Expense HTTP mutation 接入统一 Ledger，P1-B 已有数据库仲裁幂等与 20 路并发证据，P1-C 已完成服务端 Import batch/hash、原子 confirm 和前端迁移，P1-D 已完成 Recurring occurrence exactly-once。P1-E-01 提交 `5a564c9` 已让 text chat 与 OCR 一样只返回 `proposedActions`，确认前不调用 `executeActions`，并复用现有前端编辑/确认卡；该安全边界为 PASS-MOCK，AIProposal 持久化和原子确认仍未实现。后端 build 通过；integration 历史基线 8 suites / 47 tests；coverage 执行 42 suites / 344 tests 且行为断言全过，但 branch 47.63% 未达 60%；前端现有 AI confirmation 2 tests 通过。对象/外部基础设施、populated upgrade/restore、E2E、staging/release 仍未完成。
...
| backend coverage | FAILED | 42 suites / 344 tests 行为断言通过；coverage 为 statements 67.47%、branches 47.63%、functions 64.16%、lines 68.01%，全局 branch threshold 60% 未达标；OCR/MinIO fixtures 有既有 console.warn |
...
| Phase 1 功能 | PASS-MOCK + PASS-REAL | app/server/db、Ledger/coordinator、Income/Expense、Import 和 Recurring 已验证；text AI 现在确认前零 executor 调用并进入现有 proposed-action UI（PASS-MOCK）。AIProposal/Item、原子确认、upload 外部路径、对象隔离、Compose/E2E 和 release observation 仍未闭环 |
