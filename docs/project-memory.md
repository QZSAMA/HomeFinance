...
| Human confirmation | AI-proposed writes require explicit confirmation | Text chat and Image/OCR now return proposedActions without pre-confirmation execution; server-owned proposal persistence and atomic confirmation remain open |
...
Phase 1 implementation has started on `codex/phase1-ledger-trust`. Process lifecycle separation, the Ledger/coordinator protocol, Income/Expense route adoption, atomic Import confirmation, Recurring exactly-once execution, text-AI pre-confirmation safety, and the family authorization matrix are implemented with focused TDD. Import and Recurring no longer remain open transactional-adoption risks, and text AI no longer writes before confirmation. Server-owned AI proposal persistence/atomic confirmation, object/external-dependency paths, populated upgrade/restore, browser E2E, staging/release and global branch coverage remain open.
