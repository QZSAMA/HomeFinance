...
- P1-B-05 adds PASS-REAL verification only, with no production-code change: in local PostgreSQL 18.1, twenty identical concurrent `createIncome` commands advance `Family.cacheVersion` exactly once, while replaying a completed command leaves Income, IdempotencyRecord, AuditEvent and `Family.cacheVersion` unchanged. The shared-family integration assertions use relative counts so they are independent of previously committed fixtures. The full integration suite now passes 3 suites / 27 tests; the default 34-suite / 281-test coverage run still fails only the unchanged 60% branch threshold at 46.01%. This evidence does not prove HTTP route adoption, Redis/Compose behavior, populated upgrades/restores, or staging/release operation; P1-B-04's historic unclassified `INTERNAL_ERROR` keeps both gates AT_RISK.
