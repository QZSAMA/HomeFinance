...
- The default backend suite excludes every opt-in `*.integration.test.ts` file on Windows and POSIX; only exact `RUN_INTEGRATION=1` enables them, and the dedicated script discovers all integration suites. All 33 default suites / 276 tests pass. The Prisma financial-mutation adapter rejects invalid Dates, unsupported JSON values, and non-finite (`NaN`/`±Infinity`) numbers before any persistence call; its unit contract reaches 86.27% statements, 87.5% branches, 75% functions and 87.17% lines. The required global coverage command reports statements 62.16%, branches 45.23%, functions 62.18% and lines 62.58%, so it still fails only because branches are below the unchanged 60% threshold. Existing OCR/MinIO fixtures emit console warnings. Prisma format/validate/generate and the dedicated PostgreSQL integration suite pass with matching local 5.22 engines.
