...
- The default backend suite excludes every opt-in `*.integration.test.ts` file on Windows and POSIX; only exact `RUN_INTEGRATION=1` enables them, and the dedicated script discovers all integration suites. All 33 default suites / 273 tests pass. The new Prisma financial-mutation adapter unit contract raises statements to 62.14%, functions to 62.18% and lines to 62.58%, but the required coverage command still fails because branches are 45.18% against the unchanged 60% threshold. Existing OCR/MinIO fixtures emit console warnings. Prisma format/validate/generate and the dedicated PostgreSQL integration suite pass with matching local 5.22 engines.
