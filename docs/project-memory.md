...
- The default backend suite excludes every opt-in `*.integration.test.ts` file on Windows and POSIX; only exact `RUN_INTEGRATION=1` enables them, and the dedicated script discovers all integration suites. All 32 default suites / 269 tests pass, but the required coverage command fails because statements are 60.49%, branches 44.00%, functions 57.50%, and lines 61.21%; branch/function thresholds are 60%. Existing OCR/MinIO fixtures emit console warnings. Prisma format/validate/generate and the dedicated PostgreSQL integration suite pass with matching local 5.22 engines.
