...
Phase 1 implementation has started on `codex/phase1-ledger-trust`. Process lifecycle separation, the pure Ledger/coordinator protocol, additive persistence models, typed Prisma adaptation, pre-audit mutation-result validation, database-arbitrated twenty-way replay, Income/Expense POST/PUT/DELETE route adoption, and the all-entry family authorization negative matrix are implemented with focused TDD and local PostgreSQL evidence. Authenticated-member Income/Expense CRUD and unauthenticated/non-member/viewer/unknown-role rejection now have real HTTP-to-PostgreSQL evidence; positive full-role coverage for non-ledger resources, import/recurring/AI adoption, populated upgrade/restore, staging/release and global coverage evidence remain open.
