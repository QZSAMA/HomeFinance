...
Phase 1 implementation has started on `codex/phase1-ledger-trust`. Process lifecycle separation, the Ledger/coordinator protocol, Income/Expense route adoption, atomic Import confirmation, Recurring exactly-once execution, text-AI pre-confirmation safety, AI proposal persistence schema, and the family authorization matrix are implemented with focused TDD. Import and Recurring no longer remain open transactional-adoption risks, text AI no longer writes before confirmation, and proposal lifecycle facts now have an additive PostgreSQL contract. Chat/OCR proposal persistence, atomic confirmation, object/external-dependency paths, production-like populated restore, browser E2E, staging/release and global branch coverage remain open.
