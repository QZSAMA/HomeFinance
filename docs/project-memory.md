...
Phase 1 implementation has started on `codex/phase1-ledger-trust`. Process lifecycle separation, the Ledger/coordinator protocol, additive persistence models, database-arbitrated replay, Income/Expense route adoption, atomic Import batch confirmation/client migration, Recurring exactly-once occurrence execution, and the all-entry family authorization negative matrix are implemented with focused TDD and local PostgreSQL evidence. Import and Recurring no longer remain open transactional-adoption risks. AI proposal-only confirmation, object/external-dependency paths, populated upgrade/restore, browser E2E, staging/release and global branch coverage remain open.
