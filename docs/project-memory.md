...
| Atomic transaction generation | Imports, recurring, and AI writes are atomic/idempotent | Import batch confirm and recurring occurrence execution now use transaction-backed Ledger paths with local PostgreSQL atomic/replay/concurrency evidence; AI remains open |
| Human confirmation | AI-proposed writes require explicit confirmation | Text chat and Image/OCR return proposedActions without pre-confirmation execution; server-owned proposal schema is PASS-REAL, while route persistence and atomic confirmation remain open |
...
Phase 1 implementation has started on `codex/phase1-ledger-trust`. Process lifecycle separation, the Ledger/coordinator protocol, Income/Expense route adoption, atomic Import confirmation, Recurring exactly-once execution, text-AI pre-confirmation safety, AI proposal schema and chat/OCR proposal-only persistence, and the family authorization matrix are implemented with focused TDD. Import and Recurring no longer remain open transactional-adoption risks, text and OCR proposal-producing paths no longer write financial facts before confirmation, and proposal lifecycle facts now have an additive PostgreSQL contract. Atomic confirmation, object/external-dependency paths, production-like populated restore, browser E2E, staging/release and global branch coverage remain open; the raw `/execute-actions` compatibility bypass is still a P1-E-04 risk.
